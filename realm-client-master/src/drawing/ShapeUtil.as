...
public class ShapeUtil {
    public function ShapeUtil() {
        throw new ArgumentError("The ShapeUtil Class cannot be instanicated.");
    }

    public static const HORIZONTAL_LINE:String = "ShapeUtil.horizontal";
    public static const VERTICAL_LINE:String = "ShapeUtil.vertical";
    ...
    public static function drawArc(target:Graphics, x:Number, y:Number, radius:Number, arc:Number, startAngle:Number=0, yRadius:Number=0):void
    {
        ...
        {
            throw new ArgumentError("ShapeUtil.drawArc() - too few parameters, need atleast 5.");
            return;
        }
        ...
        // for our arc.
        segs = ShapeUtil.ceil(ShapeUtil.abs(arc) / 45);
    ...
    public static function drawStar(target:Graphics, x:Number, y:Number, points:uint, innerRadius:Number, outerRadius:Number, angle:Number=0):void
    {
        ...
        if (points <= 2)
        {
            throw ArgumentError("ShapeUtil.drawStar() - parameter 'points' needs to be atleast 3");
            return;
        }
        if (points > 2)
        {
     ...
     */
    public static function drawPolygon(target:Graphics, x:Number, y:Number, sides:uint, radius:Number, angle:Number=0):Vector.<Point>
    {
        if (sides > 2)
        {
            ...
            points.push(new Point(x + (Math.cos(start) * radius), y - (Math.sin(start) * radius)));
            // draw the polygon
            for (n = 1; n <= sides; ++n)
            {
        ...
        throw ArgumentError("ShapeUtil.drawPolygon() - parameter 'sides' needs to be atleast 3");
        return null;
    }
    ...
    public static function drawBurst(target:Graphics, x:Number, y:Number, sides:uint, innerRadius:Number, outerRadius:Number, angle:Number=0):void
    {
        ...
        if (sides <= 2)
        {
            throw ArgumentError("ShapeUtil.drawBurst() - parameter 'sides' needs to be atleast 3");
            return;
        }
        if (sides > 2)
        {
    ...
    public static function drawGear(target:Graphics, x:Number, y:Number, sides:uint, innerRadius:Number=80, outerRadius:Number=4, angle:Number=0, holeSides:Number=2, holeRadius:Number=0):void
    {
        ...
        if (sides <= 2)
        {
            throw ArgumentError("ShapeUtil.drawGear() - parameter 'sides' needs to be atleast 3");
            return;
        }
        if (sides > 2)
        {
     ...
     *
     */
    public static function drawLine(target:Graphics, x:Number, y:Number, length:Number, direction:String=ShapeUtil.HORIZONTAL_LINE):void
    {
        target.moveTo(x, y);
        switch (direction)
        {
            case ShapeUtil.HORIZONTAL_LINE:
                target.lineTo(length, y);
                break;
            case ShapeUtil.VERTICAL_LINE:
                target.moveTo(x, y);
